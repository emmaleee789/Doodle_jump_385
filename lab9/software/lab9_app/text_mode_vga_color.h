...
struct TEXT_VGA_STRUCT {
	/*
	0x0000 0000 - 0x000 012BF VRAM - 2 bytes per character, 2 characters per word. 80 column x 30 row. Data format is in raster order (one line at a time). 
	0x0000 12CO - 0x0000 1FFF Unused but reserved by Platform Designer 
	0x0000 2000 - 0x0000 201F Palette- 8 words of 2 colors each, for 16-color palette 
	0x0000 2020 - 0x0000 3FFF Unused but reserved by Platform Designer 
	*/
	...
	alt_u8 skip [3392]; //0x1FFF-ROWS*COLUMNS*2
	alt_u32 PALETTE [8]; //Week 2 - extended palette
};
...
static volatile struct TEXT_VGA_STRUCT* vga_ctrl = 0x08000000;
